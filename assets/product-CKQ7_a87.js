import{H as f,a as b,g as x,u as k,T as l}from"./app-F0NVCmJo.js";import{a as C,b as P,r as S}from"./render-products-catalogue-lDbyL0n1.js";import{g as w}from"./url-TDiL1Lhk.js";const q=`<section class="product">\r
...
</section>`,T=f.compile(q),I={"Vitamins & Dietary Supplements":"category--vitamins",Minerals:"category--minerals","Pain Relief":"category--pain",Probiotics:"category--probiotics",Antioxidants:"category--antioxidants","Weight Loss":"category--weight-loss","Prenatal Vitamins":"category--prenatal-vitamins"};function L(t){const o=document.querySelector(".product-page__content");t.discount&&(t.oldPrice=t.price,t.price=(t.price*(1-t.discount/100)).toFixed(2)),t.categoryClass=I[t.category]||"category--default",o.innerHTML=T(t)}function F(){l({text:"✅ Added this product to the cart!",duration:3e3,gravity:"top",position:"right",style:{background:"linear-gradient(to right, #00b09b, #96c93d)",borderRadius:"8px",fontSize:"14px",padding:"12px 20px"},stopOnFocus:!0}).showToast()}function A(){l({text:"❌ Failed to add product to cart",duration:3e3,gravity:"top",position:"right",style:{background:"linear-gradient(to right, #ff5f6d, #ffc371)",borderRadius:"8px",fontSize:"14px",padding:"12px 20px"},stopOnFocus:!0}).showToast()}async function $(){const t=w("id");if(!t){console.error("Product ID not found");return}const o=await C(t);L(o);const _=document.querySelector(".product-advertisement__items"),h=await P({category:o.category,excludeId:o.id,limit:4});S(h,_);const g=document.querySelector(".product__backlink");localStorage.setItem("productsPageUrl",document.referrer||"./products.html"),g.addEventListener("click",e=>{e.preventDefault();const c=localStorage.getItem("productsPageUrl")||"./products.html";window.location.href=c});const m=document.querySelector(".product__cart-btn-plus"),v=document.querySelector(".product__cart-btn-menus"),i=document.querySelector(".product__cart-count");let a=document.querySelector(".product__price--regular");a||(a=document.querySelector(".product__price--new"));const d=document.querySelector(".product__autoship-checkbox"),p=document.querySelector(".product__autoship");let n=d.checked;d.addEventListener("change",e=>{n=e.target.checked,n?(p.classList.add("product__autoship--active"),console.log(n)):(p.classList.remove("product__autoship--active"),console.log(n))});const s=document.querySelector(".product__autoship-select");s.addEventListener("change",e=>{console.log(s.value)});const y=a.textContent,u=parseFloat(y.replace("$","").trim());let r=1;m.addEventListener("click",()=>{r+=1,i.textContent=r;const e=u*r;a.textContent=`$${e.toFixed(2)}`}),v.addEventListener("click",()=>{r>1&&(r-=1,i.textContent=r);const e=u*r;a.textContent=`$${e.toFixed(2)}`}),document.querySelector(".product__order-btn").addEventListener("click",async()=>{const e=await b({product:o,quantity:r,autoshipStatus:n,autoshipInterval:n?s.value:null});if(console.log(e),e&&!e.error){F();const c=await x();k(c)}else A()})}export{$ as initProduct};
